...
internal class PriceChart
{
	...
	public void Create(List<OHLC> data, Stream output)
	{
		...
		canvas.DrawImage(SKImage.FromBitmap(SKBitmap.Decode(Assembly.GetExecutingAssembly().GetManifestResourceStream("PriceChartBot.Resources." + ImageResource))), 0, 0);
		...
		for (int x = 0; x < w; x++)
		{
			...
			if (prev_h != Math.Floor(t.Hour / hourStep))
			{
				if (t.Hour == 0)
				{
					...
					var tw = paintFont.MeasureText(t.ToString("d MMM", CultureInfo.InvariantCulture));
					canvas.DrawText(t.ToString("d MMM", CultureInfo.InvariantCulture), x + paddingLeft + indentX - tw / 2, height - paddingBottom + hourTickHeight * 3 + 2, font, paintFont);
				}
